function [net,stats] = cnn_train_dag_regressor(net, imdb, getBatch, varargin)
...
for epoch=start+1:opts.numEpochs
  ...
  if numel(opts.gpus) <= 1
    [net, state] = processEpoch(opts, net, state, params, 'train') ;
    [net, state] = processEpoch(opts, net, state, params, 'val') ;
    if ~evaluateMode
...
% -------------------------------------------------------------------------
function [net, state] = processEpoch(opts, net, state, params, mode)
% -------------------------------------------------------------------------
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    for i=1:numel(reg_layers)
       iter=((epoch-1)*opts.batchSize) + round(t/opts.batchSize)+1;
